...
function postgresDatabaseFactory ({ config, logger }) {
  ...
  async function start () {
    try {
      ...
      // Create tables using sync
      logger.info({ message: 'Syncing database schema' })
      await database.sync()
      logger.info({ message: 'Database schema synced' })
    } catch (error) {
      logger.error({
        message: 'Cannot authenticate or sync database',
        error: error.message,
        stack: error.stack ? error.stack.split('\n') : null,
      })
